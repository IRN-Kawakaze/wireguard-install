...
PresharedKey = $psk
AllowedIPs = 192.168.64.$octet/32$(grep -q 'fdc0:a840::1' /etc/wireguard/wg0.conf && echo ", fdc0:a840::$octet/128")
# END_PEER $client
EOF
	# Create client configuration
	cat << EOF > ~/"$client".conf
[Interface]
Address = 192.168.64.$octet/24$(grep -q 'fdc0:a840::1' /etc/wireguard/wg0.conf && echo ", fdc0:a840::$octet/64")
DNS = $dns
PrivateKey = $key
...
[Interface]
Address = 192.168.64.1/24$([[ -n "$ip6" ]] && echo ", fdc0:a840::1/64")
...
EOF
	...
	if systemctl is-active --quiet firewalld.service; then
		...
		firewall-cmd --zone=trusted --add-source=192.168.64.0/24
		firewall-cmd --permanent --add-port="$port"/udp
		firewall-cmd --permanent --zone=trusted --add-source=192.168.64.0/24
		# Set NAT for the VPN subnet
		firewall-cmd --direct --add-rule ipv4 nat POSTROUTING 0 -s 192.168.64.0/24 ! -d 192.168.64.0/24 -j SNAT --to "$ip"
		firewall-cmd --permanent --direct --add-rule ipv4 nat POSTROUTING 0 -s 192.168.64.0/24 ! -d 192.168.64.0/24 -j SNAT --to "$ip"
		if [[ -n "$ip6" ]]; then
			firewall-cmd --zone=trusted --add-source=fdc0:a840::/64
			firewall-cmd --permanent --zone=trusted --add-source=fdc0:a840::/64
			firewall-cmd --direct --add-rule ipv6 nat POSTROUTING 0 -s fdc0:a840::/64 ! -d fdc0:a840::/64 -j SNAT --to "$ip6"
			firewall-cmd --permanent --direct --add-rule ipv6 nat POSTROUTING 0 -s fdc0:a840::/64 ! -d fdc0:a840::/64 -j SNAT --to "$ip6"
		fi
	else
...
[Service]
Type=oneshot
ExecStart=$iptables_path -t nat -A POSTROUTING -s 192.168.64.0/24 ! -d 192.168.64.0/24 -j SNAT --to $ip
ExecStart=$iptables_path -I INPUT -p udp --dport $port -j ACCEPT
ExecStart=$iptables_path -I FORWARD -s 192.168.64.0/24 -j ACCEPT
ExecStart=$iptables_path -I FORWARD -m state --state RELATED,ESTABLISHED -j ACCEPT
ExecStop=$iptables_path -t nat -D POSTROUTING -s 192.168.64.0/24 ! -d 192.168.64.0/24 -j SNAT --to $ip
ExecStop=$iptables_path -D INPUT -p udp --dport $port -j ACCEPT
ExecStop=$iptables_path -D FORWARD -s 192.168.64.0/24 -j ACCEPT
ExecStop=$iptables_path -D FORWARD -m state --state RELATED,ESTABLISHED -j ACCEPT" > /etc/systemd/system/wg-iptables.service
		if [[ -n "$ip6" ]]; then
			echo "ExecStart=$ip6tables_path -t nat -A POSTROUTING -s fdc0:a840::/64 ! -d fdc0:a840::/64 -j SNAT --to $ip6
ExecStart=$ip6tables_path -I FORWARD -s fdc0:a840::/64 -j ACCEPT
ExecStart=$ip6tables_path -I FORWARD -m state --state RELATED,ESTABLISHED -j ACCEPT
ExecStop=$ip6tables_path -t nat -D POSTROUTING -s fdc0:a840::/64 ! -d fdc0:a840::/64 -j SNAT --to $ip6
ExecStop=$ip6tables_path -D FORWARD -s fdc0:a840::/64 -j ACCEPT
	...
	case "$option" in
		...
		3)
			...
			if [[ "$remove" =~ ^[yY]$ ]]; then
				...
				if systemctl is-active --quiet firewalld.service; then
					ip=$(firewall-cmd --direct --get-rules ipv4 nat POSTROUTING | grep '\-s 192.168.64.0/24 '"'"'!'"'"' -d 192.168.64.0/24' | grep -oE '[^ ]+$')
					# Using both permanent and not permanent rules to avoid a firewalld reload.
					firewall-cmd --remove-port="$port"/udp
					firewall-cmd --zone=trusted --remove-source=192.168.64.0/24
					firewall-cmd --permanent --remove-port="$port"/udp
					firewall-cmd --permanent --zone=trusted --remove-source=192.168.64.0/24
					firewall-cmd --direct --remove-rule ipv4 nat POSTROUTING 0 -s 192.168.64.0/24 ! -d 192.168.64.0/24 -j SNAT --to "$ip"
					firewall-cmd --permanent --direct --remove-rule ipv4 nat POSTROUTING 0 -s 192.168.64.0/24 ! -d 192.168.64.0/24 -j SNAT --to "$ip"
					if grep -qs 'fdc0:a840::1/64' /etc/wireguard/wg0.conf; then
						ip6=$(firewall-cmd --direct --get-rules ipv6 nat POSTROUTING | grep '\-s fdc0:a840::/64 '"'"'!'"'"' -d fdc0:a840::/64' | grep -oE '[^ ]+$')
						firewall-cmd --zone=trusted --remove-source=fdc0:a840::/64
						firewall-cmd --permanent --zone=trusted --remove-source=fdc0:a840::/64
						firewall-cmd --direct --remove-rule ipv6 nat POSTROUTING 0 -s fdc0:a840::/64 ! -d fdc0:a840::/64 -j SNAT --to "$ip6"
						firewall-cmd --permanent --direct --remove-rule ipv6 nat POSTROUTING 0 -s fdc0:a840::/64 ! -d fdc0:a840::/64 -j SNAT --to "$ip6"
					fi
				else
